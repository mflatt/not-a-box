(library (hash)
  (export hash hasheqv hasheq
          ...
          hash? hash-eq? hash-equal? hash-eqv? hash-count
          hash-keys-subset?
          ;; For intern tables:
          weak-hash-ref-key)
  (import (hash-code)
  ...
  (define (hash? v) (or (mutable-hash? v) (immutable-hash? v)))
    ...
    (cond
     ...
     [(immutable-hash? ht)
      (let ([ht (cond
                 ...
                 [else (make-hash)])])
        (let loop ([i (immutable-hash-iterate-first ht)])
          (when i
            (let-values ([(k v) (immutable-hash-iterate-key+value ht i)])
              (hashtable-set! ht k v)
              (loop (immutable-hash-iterate-next ht i)))))
