...
(define (find-definitions v prim-knowns knowns imports mutated)
  (match v
     ...
     (cond
      [info
       ...
       (let* ([knowns (hash-set knowns
         ...
         (values (hash-set knowns struct:s (known-struct-type type (struct-type-info-field-count info)))
                 info))]
    ...
    [`(define-values (,prop:s ,s? ,s-ref)
     ...
     (values
      (let* ([knowns (hash-set knowns s-ref (known-accessor type))]
             [knowns (hash-set knowns s? (known-predicate type))])
        ;; Check whether the property type has an immediate (or no) guard:
        (cond
         [(or (null? rest)
              (and (not (car rest))
                   (null? (cdr rest))))
          (hash-set knowns prop:s a-known-struct-type-property/immediate-guard)]
         [else knowns]))
      #f)]
    [`,_ (values knowns #f)]))
...
(define (lambda? v)
  (match v
    [`(lambda . ,_) #t]
    [`(case-lambda . ,_) #t]
    [`(let-values ([(,id) ,rhs]) ,body) (or (and (eq? id body) (lambda? rhs))
                                            (lambda? body))]
    [`(letrec-values ([(,id) ,rhs]) ,body) (or (and (eq? id body) (lambda? rhs))
                                               (lambda? body))]
